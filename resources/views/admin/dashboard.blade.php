...
<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div class="bg-pink-600 text-white p-6 rounded-xl shadow-md flex flex-col justify-center items-center transform transition hover:scale-105">
            <h5 class="text-lg font-medium text-pink-100 mb-1">Total Aspirasi Masuk</h5>
            ...
            <ul class="divide-y divide-gray-200">
                @foreach($kategoriTerbanyak as $item)
                    <li class="py-3 flex justify-between items-center">
                        <span class="text-gray-800 font-medium">{{ $item->kategori->ket_kategori }}</span>
                        <span class="bg-pink-100 text-pink-800 text-sm font-bold px-3 py-1 rounded-full shadow-sm">{{ $item->total }} laporan</span>
                    </li>
                @endforeach
            </ul>
        @else
