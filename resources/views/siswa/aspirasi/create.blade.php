...
<div class="max-w-3xl mx-auto">
    ...
    <form action="{{ route('aspirasi.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="lokasi" id="lokasi" class="w-full border border-gray-300 rounded-md p-2.5 focus:ring-2 focus:ring-pink-500 focus:border-pink-500 outline-none transition" value="{{ old('lokasi') }}" placeholder="Contoh: Kantin, Lab RPL, Parkiran" required>
        </div>

        <div>
            <label for="keterangan" class="block text-sm font-medium text-gray-700 mb-1">Detail Aspirasi</label>
            <textarea name="keterangan" id="keterangan" rows="5" class="w-full border border-gray-300 rounded-md p-2.5 focus:ring-2 focus:ring-pink-500 focus:border-pink-500 outline-none transition" placeholder="Ceritakan detail aspirasi atau keluhan Anda..." required>{{ old('keterangan') }}</textarea>
        </div>
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            <button type="submit" class="px-5 py-2.5 bg-pink-600 text-white rounded-md hover:bg-pink-700 transition font-semibold shadow-md">Kirim Aspirasi</button>
        </div>
    </form>
</div>
@endsection